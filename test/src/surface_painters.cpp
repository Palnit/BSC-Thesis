...
void DrawLine(SDL_Surface *surface, RGBA color, glm::vec2 start, glm::vec2 end) {
    ...
    while (true) {
        ...
        RGBA* pixel = (RGBA*) (((uint8_t*) surface->pixels) + ((int) start.x * 4)
...
void SurfacePainters::DrawLine(SDL_Surface *surface, glm::vec2 start,
                               glm::vec2 end) {
    float dx = abs(end.x - start.x);
    int sx = start.x < end.x ? 1 : -1;
    float dy = -abs(end.y - start.y);
...
void DrawCubicBezier(SDL_Surface *surface, RGBA color, glm::ivec2 p1,
                     glm::ivec2 p2, glm::ivec2 p3, glm::ivec2 p4) {
