...
std::vector<cl::Device>& OpenCLInfo::GetOpenCLInfoAndDevices() {

    if (OpenCLInfo::OPENCL_DEVICES.empty()) {
        ...
        for (const auto& platform : platforms) {
            ...
            for (const auto& j : devices_available) {
                OpenCLInfo::OPENCL_DEVICES.push_back(j);
            }
        }
    }
    ...
    if (OpenCLInfo::OPENCL_DEVICES.empty()) {
        ...
        fprintf(stderr, "Found %llu OpenCl Capable device(s)\n",
                OpenCLInfo::OPENCL_DEVICES.size());
    }

    for (int i = 0; i < OpenCLInfo::OPENCL_DEVICES.size(); i++) {
        fprintf(stderr,
                "\nDevice %d: \"%s\"\n",
                i,
                OpenCLInfo::OPENCL_DEVICES[i].getInfo<CL_DEVICE_NAME>()
                    .c_str());

        fprintf(stderr,
                "  Vendor                   :\t%s\n",
                OpenCLInfo::OPENCL_DEVICES[i].getInfo<CL_DEVICE_VENDOR>()
                    .c_str());
        fprintf(stderr,
                "  Driver Version           :\t%s\n",
                OpenCLInfo::OPENCL_DEVICES[i].getInfo<CL_DRIVER_VERSION>()
                    .c_str());

        auto type = OpenCLInfo::OPENCL_DEVICES[i].getInfo<CL_DEVICE_TYPE>();
